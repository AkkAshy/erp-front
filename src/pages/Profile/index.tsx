...
const Profile = () => {
  ...
  useEffect(() => {
    if (profile.data) {
      setName(profile.data?.data.first_name);
      setSurname(profile.data?.data.last_name);
      const phoneNumber = profile.data?.data?.employee?.phone;
      if (phoneNumber) {
        setPhone(format(phoneNumber.replace(/^\+998/, ""), maskOptions));
      } else {
        setPhone(""); // Set default empty string if no phone
      }

      setGender(profile.data?.data?.employee?.sex);
    }
  }, [profile.data]);
