    ...
    <div className={styles.category}>
      ...
      <CreateModal
        ...
        btnOnClick={() => {
          addCategory
            .mutateAsync({
              name: categoryValue,
              attribute_types: selectedAttributeTypes,
            })
            .then((res) => {
              if (res.status === 201) {
      ...
      >
        ...
        <div className={styles.attributesSection}>
          <label>Атрибуты для категории</label>
          <div className={styles.attributesList}>
            {attributeTypes.data?.data.results.map((attrType) => (
              <label key={attrType.id} className={styles.checkboxLabel}>
                <input
                  type="checkbox"
                  checked={selectedAttributeTypes.includes(attrType.id)}
                  onChange={(e) => {
                    if (e.target.checked) {
                      setSelectedAttributeTypes([...selectedAttributeTypes, attrType.id]);
                    } else {
                      setSelectedAttributeTypes(
                        selectedAttributeTypes.filter((id) => id !== attrType.id)
                      );
                    }
                  }}
                />
                <span>{attrType.name}</span>
              </label>
            ))}
          </div>
        </div>
            ...
            content_1: (
              <div
                onClick={() => {
                  ...
                  // Извлекаем ID из attribute_types_detail
                  const attributeIds = item.attribute_types_detail?.map(attr => attr.id) || [];
                  setSelectedAttributeTypes(attributeIds);
                  setEditId(item.id);
                }}
              >
                <EditIcon />
