...
const Home = () => {
    ...
    setProducts((prev) =>
      prev.map((item) => {
        if (item.id === id) {
          ...
          if (newCount > item.current_stock) return item;
  ...
  useEffect(() => {
    if (scanBarcode.data?.data.product) {
      ...
      if (!productItems.current_stock || productItems.current_stock === 0) {
      ...
      setProducts((prev) => {
        ...
        if (existing) {
          ...
          if (currentCount >= productItems.current_stock) {
              ...
              return {
                ...
                quantity: (
                  <div className={styles.quantity__action}>
                    ...
                    <button
                      disabled={(item.count ?? 1) === item.current_stock}
                      className={styles.action__btn}
